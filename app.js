// QR-Reader v7.3.4 (vendor-less core). Multi-engine scan + robust Tesseract + red OCR boxes.
(function(){
  'use strict';
  ...
  var seenEver = new Set? new Set() : { _m:{}, add:function(v){this._m[v]=1;}, has:function(v){return !!this._m[v];}, clear:function(){this._m={};} };
  var lastOCRBoxes=[];
  ...
  function ensureZXingWASM(){
    if (ZXING_WASM_LOADED || (window.ZXingWASM && (window.ZXingWASM.readBarcodesFromImageData || window.ZXingWASM.instantiate || window.ZXingWASM.decode))) {
  ...
  function recognizeSimple(canvas){
    return ensureTesseract().then(function(ok){
      ...
      return window.Tesseract.recognize(canvas, 'eng', tesseractOpts())
        ...
        .catch(function(){
          try{ return window.Tesseract.recognize(canvas, { lang:'eng' }).then(extractTextAndBoxes).catch(function(){ return { text:'', boxes:[] }; }); }
          catch(_e){ return { text:'', boxes:[] }; }
        });
    });
  }
  ...
  function sizeOverlay(){
    ...
    var r=video.getBoundingClientRect(), dpr=window.devicePixelRatio||1;
  ...
  function startFromSelection(){
    if(cameraSourceSel.value==='remote'){ setStatus('Remote camera active (stub).'); return; }
    if(cameraSourceSel.value==='serial'){ setStatus('Serial source active (stub).'); return; }
      ...
      try{
        ...
        if(window.BarcodeDetector.getSupportedFormats){
          window.BarcodeDetector.getSupportedFormats().then(function(supported){
            var fmts=[]; for(var i=0;i<desired.length;i++){ if(supported.indexOf(desired[i])!==-1) fmts.push(desired[i]); }
  ...
  function handleDetection(text, fmt){
    ...
    if(ignoreDup && (seenEver.has?seenEver.has(text):seenEver._m[text])) return;
    ...
    if(ignoreDup){ if(seenEver.add) seenEver.add(text); else seenEver._m[text]=1; }
  ...
  if(resetDupBtn){ resetDupBtn.addEventListener('click', function(){ if(seenEver.clear) seenEver.clear(); else seenEver._m={}; toast('Duplicate memory cleared.'); }); }
  ...
  clearBtn.addEventListener('click', function(){ if(confirm('Clear all rows?')){ data=[]; save(); render(); if(seenEver.clear) seenEver.clear(); else seenEver._m={}; }});
  ...
  if (testOCRBtn){
    testOCRBtn.addEventListener('click', function(){
      ...
      recognizeCanvasSmart(snap, function(txt, boxes){
        ...
        lastOCRBoxes = boxes || [];
        drawROI();
        var msg = txt.trim? (txt.trim().slice(0,120).replace(/\s+/g,' ')) : '';
        toast('OCR sample: '+(msg||'(no text)'));
      });
    });
  }
  ...
  // Prefs: default OCR on if scaleMode was ocr (or no hardware scale)
  var prefs = loadPrefs();
