...
(function(){
  ...
  let scanTimer=null;
  const roi = { x:0.58, y:0.58, w:0.40, h:0.38, show:false };

  // zxing-wasm runtime
  let zxwReady=false;

  // Tesseract worker (local assets)
  ...
  if(clearBtn) clearBtn.addEventListener('click', function(){ if(confirm('Clear all rows?')){ data=[]; save(); render(); } });
  ...
  ocrToggleBtn.addEventListener('click', function(){
    ...
    drawROI();
    if(roi.show && (!scaleModeSel || scaleModeSel.value!=='ocr')){
      setStatus('Tip: set Scale source to "OCR from camera" to read the box.');
    }
  });
})();