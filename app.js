...
(function(){
  ...
  headExists('vendor/zxing-wasm-reader.iife.js').then(function(ok){
    ...
    return loadScript('vendor/zxing-wasm-reader.iife.js').then(function(ok2){
      if(ok2 && window.ZXingWASM && typeof ZXingWASM.prepareZXingModule==='function'){
        try{ ZXingWASM.prepareZXingModule({overrides:{locateFile:function(path,prefix){ return (/\.wasm$/i).test(path)?'vendor/zxing_reader.wasm':(prefix||'')+path; }}}); }catch(e){}
      }
      return ok2;
    });
  });
  ...
  function moveDrag(ev){ if(!dragging) return; var p=norm(ev), dx=p.nx-dragging.ox, dy=p.ny-dragging.oy, minW=0.08, minH=0.08; if(dragging.mode==='move'){ roi.x=Math.max(0,Math.min(1-dragging.rw,dragging.rx+dx)); roi.y=Math.max(0,Math.min(1-dragging.rh,dragging.ry+dy)); } else { var x=dragging.rx, y=dragging.ry, w=dragging.rw, h=dragging.rh; if(dragging.mode.indexOf('n')>=0){ y=Math.max(0,Math.min(dragging.ry+dy,dragging.ry+dragging.rh-minH)); h=(dragging.ry+dragging.rh)-y; } if(dragging.mode.indexOf('s')>=0){ h=Math.max(minH,Math.min(1-dragging.ry,dragging.rh+dy)); } if(dragging.mode.indexOf('w')>=0){ x=Math.max(0,Math.min(dragging.rx+dx,dragging.rx+dragging.rw-minW)); w=(dragging.rx+dragging.rw)-x; } if(dragging.mode.indexOf('e')>=0){ w=Math.max(minW,Math.min(1-dragging.rx,dragging.rw+dx)); } roi.x=x; roi.y=y; roi.w=w; roi.h=h; } drawOverlay(); if(ev.preventDefault) ev.preventDefault(); }
  ...
  roi.show=true; startOcrPulse(); setOCRStatus('On (idle)');
